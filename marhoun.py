...
def info():
    while True:
        ...
        info_text = get_font(45).render("Game is gonna be here soon.", True, "White")
        play_rect = info_text.get_rect(center=(700, 350))
        glasses.blit(info_text, play_rect)
        info_back = Button(image=None, pos=(700, 550),
                           text_input="BACK", font=get_font(75), base_color="White", hovering_color="Green")

        info_back.changeColor(game_mouse_pos)
        ...
        for event in pg.event.get():
            ...
            if event.type == pg.MOUSEBUTTONDOWN:
                if info_back.checkForInput(game_mouse_pos):
...
while running:
    ...
    my_image = pg.image.load('Sprites/crosshair_hit.png').convert_alpha()
    info_button = Button(image=pg.image.load("assets/Play Rect.png"), pos=(1300, 600),
                text_input="photo here", font=get_font(20), base_color="#d7fcd4", hovering_color="White")
    info_button.update(glasses)
    for event in pg.event.get():
        if event.type == pg.QUIT:
            running = False
        if event.type == pg.MOUSEBUTTONDOWN:
            if event.button == 1:
                ...
                for i in range(len(Duck.ducks)):
                    if Duck.ducks[i][0].x + 30 <= mouse_x <= Duck.ducks[i][0].x + 150 and \
                        ...
                        Duck.death(i)
                        break
            if info_button.checkForInput(game_mouse_pos):
                info()
